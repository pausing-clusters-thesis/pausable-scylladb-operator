...
func (p *Proxy) handleConnection(ctx context.Context, conn net.Conn) {
	...
	if !ok {
		...
		klog.V(5).InfoS("Waiting for PausableScyllaDBDatacenter to reach Available condition.", "ServerName", sniHost, "Ref", klog.KObj(psc))
		...
		_, err = controllerhelpers.WaitForPausableScyllaDBDatacenterState(
			...
			isPausableScyllaDBDatacenterAvailable,
		)
		if err != nil {
			klog.ErrorS(err, "can't wait for PausableScyllaDBDatacenter to become available", "ServerName", sniHost, "Ref", klog.KObj(psc))
			return
		}

		klog.V(5).InfoS("PausableScyllaDBDatacenter reached Available condition, trying to find a route.", "ServerName", sniHost, "Ref", klog.KObj(psc))
...
func isPausableScyllaDBDatacenterAvailable(psdc *pausingv1alpha1.PausableScyllaDBDatacenter) (bool, error) {
	return sohelpers.IsStatusConditionPresentAndTrue(psdc.Status.Conditions, pausingv1alpha1.AvailableCondition, psdc.Generation), nil
}
