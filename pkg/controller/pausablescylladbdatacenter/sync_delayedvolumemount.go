...
func (psdcc *Controller) syncDelayedVolumeMount(ctx context.Context, psdc *pausingv1alpha1.PausableScyllaDBDatacenter, scyllaDBDatacenterClaims map[string]*pausingv1alpha1.ScyllaDBDatacenterClaim) ([]metav1.Condition, error) {
	...
	if err != nil {
		return progressingConditions, fmt.Errorf("can't get ScyllaDBDatacenter %q: %w", naming.ManualRef(psdc.Namespace, *sdcc.Status.ScyllaDBDatacenterName), err)
	}

	for _, r := range sdc.Spec.Racks {
		rackNodeCount, err := socontrollerhelpers.GetRackNodeCount(sdc, r.Name)
		if err != nil {
			return progressingConditions, fmt.Errorf("can't get rack node count for rack %q: %w", r.Name, err)
		}
		...
		for i := range *rackNodeCount {
			...
			if err != nil {
				return progressingConditions, fmt.Errorf("can't get persistent volume claim %q: %w", naming.ManualRef(psdc.Namespace, proxyPVCName), err)
			}
			...
			if err != nil {
				return progressingConditions, fmt.Errorf("can't set annotation: %w", err)
			}
		}
	}
