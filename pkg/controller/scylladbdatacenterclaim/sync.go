...
func (sdccc *Controller) sync(ctx context.Context, key string) error {
	// TODO: add progressing conditions, split logic to separate funcs?
	...
	//	&status.Conditions,
	//	scyllaDBDatacenterBindControllerProgressingCondition,
	//	scyllaDBDatacenterBindControllerDegradedCondition,
	//	sdcc.Generation,
	//	func() ([]metav1.Condition, error) {
	//		return sdccc.syncScyllaDBDatacenterBind(ctx, sdcc, status, scyllaDBDatacenters)
	//	},
	//)
	//if err != nil {
	//	errs = append(errs, fmt.Errorf("can't sync scylla cluster binding: %w", err))
	//}
	...
	if err != nil {
		errs = append(errs, fmt.Errorf("can't sync scylla clusters: %w", err))
	}
