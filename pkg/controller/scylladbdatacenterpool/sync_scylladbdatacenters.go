...
func (sdcpc *Controller) syncScyllaDBDatacenters(
	...
	// TODO: sort by readiness, bootstrap phase, etc.
		...
		if controllerhelpers.HasAnnotation(sdcc, naming.ScyllaDBDatacenterClaimBindCompletedAnnotation) {
			...
			if sdcc.Status.ScyllaDBDatacenterName == nil || len(*sdcc.Status.ScyllaDBDatacenterName) == 0 {
				...
				if len(unclaimedScyllaDBDatacenters) == 0 {
					...
					sdc := makeScyllaDBDatacenter(sdcp, sdcpc.proxyStorageClassName)
					...
					sdcName := fmt.Sprintf("scylladbdatacenter-%s", sdcc.Name)
					sdc.Name = sdcName

					sdccCopy := sdcc.DeepCopy()
					sdccCopy.Status.ScyllaDBDatacenterName = pointer.Ptr(sdcName)

					sdccCopy, err = sdcpc.pausableScyllaDBClient.ScyllaDBDatacenterClaims(sdcc.Namespace).UpdateStatus(ctx, sdccCopy, metav1.UpdateOptions{})
					if err != nil {
					...
					if err != nil {
						return progressingConditions, fmt.Errorf("can't create ScyllaDBDatacenter %q: %w", naming.ManualRef(sdcc.Namespace, sdcName), err)
					}
					...
					_, err = sdcpc.pausableScyllaDBClient.ScyllaDBDatacenterClaims(sdcc.Namespace).Update(ctx, sdccCopy, metav1.UpdateOptions{})
					if err != nil {
						return progressingConditions, fmt.Errorf("can't update ScyllaDBDatacenterClaim %q status: %w", sonaming.ObjRef(sdccCopy), err)
					}
				} else {
				...
				if ok {
					...
					if err != nil {
						...
						_, err = sdcpc.pausableScyllaDBClient.ScyllaDBDatacenterClaims(sdcc.Namespace).UpdateStatus(ctx, sdccCopy, metav1.UpdateOptions{})
						if err != nil {
							return progressingConditions, fmt.Errorf("can't update ScyllaDBDatacenterClaim %q status: %w", sonaming.ObjRef(sdccCopy), err)
						}
	...
	if capacityDiff > 0 {
		klog.V(2).InfoS("Too few ScyllaDBDatacenters in the pool", "ScyllaDBDatacenterPool", klog.KObj(sdcp), "capacity", sdcp.Spec.Capacity, "got", len(unclaimedScyllaDBDatacenters), "creating", capacityDiff)

		scyllaDBDatacenterTemplate := makeScyllaDBDatacenter(sdcp, sdcpc.proxyStorageClassName)
		...
		limitDiff *= -1
		klog.V(2).InfoS("Too many ScyllaDBDatacenters in the pool", "ScyllaDBDatacenterPool", klog.KObj(sdcp), "limit", sdcp.Spec.Limit, "got", len(unclaimedScyllaDBDatacenters), "pruning", limitDiff)

		// TODO: delete concurrently
		// TODO: implement SDC selection for pruning (readiness, bootstrap phase, colocation...)
...
func makeScyllaDBDatacenter(sdcp *pausingv1alpha1.ScyllaDBDatacenterPool, proxyStorageClassName string) *scyllav1alpha1.ScyllaDBDatacenter {
	return &scyllav1alpha1.ScyllaDBDatacenter{
		...
		Spec: func() scyllav1alpha1.ScyllaDBDatacenterSpec {
			...
			for i := range specCopy.Racks {
				...
				specCopy.Racks[i].ScyllaDB.Storage.StorageClassName = pointer.Ptr(proxyStorageClassName)
			}
